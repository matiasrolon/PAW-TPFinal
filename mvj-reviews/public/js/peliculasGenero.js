...
PeliculaGenero.buildGrid = function (response) {
  ...
  let c = PeliculaGenero.crearElemento("section", ["peliculas-genero"]);
  c.setAttribute("onscroll","PeliculaGenero.getNextChunckScroll()");
  b.appendChild(c);
  for (var ii in resp) {
    ...
    let l = PeliculaGenero.crearElemento("p", ["titulo-film"]);
...
PeliculaGenero.initialize = function (genre, category, container) {
  ...
  let nom = document.querySelector(".generos li div[id='"+PeliculaGenero.genre+"+']");
