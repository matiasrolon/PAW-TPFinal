...
Novelties.startNovelties = function (contenedorHTML) {
  window.addEventListener("DOMContentLoaded", function () {
    ...
    // Si hay errores, hago que los errores se eliminen al hacerles click. 
    if ( document.getElementById('mensajes').childElementCount > 0 ) {
      var liErrors = document.querySelectorAll('#mensajes li');
      var i = 0;
      for (i=0; i < liErrors.length; i++) {
        Novelties.borrarOnClick(liErrors[i]);
      }
    }
...
Novelties.borrarOnClick = function(elemento) {
  if (elemento) {
...
Novelties.changeNoveltieCreate = function (type) {
  ...
  // Muestro el menu de herramientas
  var toolbox = document.getElementById('toolbox');
  toolbox.classList.remove('no-visible');

  // Elimino los errores dentro de la lista de #mensajes
  var liErrors = document.querySelectorAll('#mensajes li');
  var i = 0;
  for (i=0; i < liErrors.length; i++) {
    liErrors[i].remove();
